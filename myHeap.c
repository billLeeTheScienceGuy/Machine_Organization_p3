...
void* myAlloc(int size) {     
    ...
    int reqSize = size + 4;
    //if size is not a positive integer, return NULL.
    if(size <= 0){
	    return NULL;
    }
    // 
    if(size >= curr->size_status - (curr->size_status % 8)){
		    return NULL;
		    }
    while(!(reqSize % 8 == 0)){
	    reqSize++;
    ...
    while(!(curr == NULL)){
		    if((curr->size_status & 1)  && (curr->size_status >= reqSize + sizeof(blockHeader))){
			    if ((curr->size_status) % 8 == 0){
				    int sizeSecBlock = curr->size_status - reqSize;
				    curr->size_status = reqSize;
				    blockHeader *free = curr + size;
				    free->size_status = sizeSecBlock;
			    }
			    best = curr;
			    best->size_status = curr->size_status;
		    }

		    curr = (blockHeader*)((char*)curr + curr->size_status);
    }
} 
...
int myFree(void *ptr) {    
   ...
   curr = ptr - sizeof(blockHeader);
   //Return -1 if ptr block is already freed.
   if(curr->size_status & 1){
	   return -1;
   }
   int ptrLoc = curr - heapStart;
   //Return -1 if ptr is outside of the heap space.
   if(ptrLoc >= allocsize || ptrLoc < 0){
	   return -1;
   }
   //Returns 0 on success.
   return 0;
